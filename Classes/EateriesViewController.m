...
- (id)init{
    if (self = [super init]) {
		...
		NSLog(@"timeIntervalSinceNow:%f",age);
		if(age > -0.01) {
			NSLog(@"Less than 3");
		...
		NSLog(@"mypresentLocation lat:%f , long:%f, last loggedtime:%@",myLocationManager.location.coordinate.latitude,myLocationManager.location.coordinate.longitude,myLocationManager.location.timestamp);
